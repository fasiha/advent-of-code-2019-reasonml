...
let sum = l => List.fold_left((memo, curr) => memo + curr, 0, l);
let masses = [
  127117,
  121199,
  80494,
...
let answer1 = sum(List.map(naiveMassToFuel, masses));
Js.log(answer1);

let rec massToFuel = x =>
  switch (x) {
  | n when n <= 0 => x
  | n =>
    let f = naiveMassToFuel(n);
    f + massToFuel(f);
  };
// Js.log(massToFuel(100756));
let answer2 = sum(List.map(massToFuel, masses));
Js.log(answer2);

let intcode = original => {
  let program = Js.Array.copy(original);
  let posRef = ref(0);
  while (program[posRef^] != 99) {
    let pos = posRef^;
    let leftPos = program[pos + 1];
    let rightPos = program[pos + 2];
    let destPos = program[pos + 3];
    if (program[pos] == 1) {
      program[destPos] = program[leftPos] + program[rightPos];
    } else {
      program[destPos] = program[leftPos] * program[rightPos];
    };
    posRef := pos + 4;
...
// Js.log(Array.of_list(Stream.npeek(55, Stream.from(n => Some(n)))));

let cartesian = (v, w) =>
  List.concat(List.map(ve => List.map(we => (ve, we), w), v));
...
// Js.log(Array.of_list(cartesian([1, 2, 3], [5, 6, 7])));

{
  let program1 = [|1, 9, 10, 3, 2, 3, 11, 0, 99, 30, 40, 50|];
  let program2 = [|1, 1, 1, 4, 99, 5, 6, 0, 99|];
  // Js.log(intcode(program1));
  // Js.log(intcode(program2));
  let program = [|
    1,
    0,
    0,
  ...
  let (noun, verb) =
    List.find(
      tup => {
        ...
        program[1] = noun;
        program[2] = verb;
        intcode(program)[0] == 19690720;
      },
      cartesian(range(0, 100), range(0, 100)),
    );
  let answer4 = 100 * noun + verb;
  Js.log(answer4);
};
